...
- ✅ **Brochure deployado na VPS Hostinger** (Ubuntu 26.04 LTS · IP 177.7.45.10 · KVM · sem domínio). Caddy escutando :3000 servindo `/var/www/qg-brochure/` (build estático). UFW liberada pra 22 + 3000. SSH key auth configurada (chave do Cowork em ~/.ssh/qg_vps no Mac do Augusto, alias SSH `qg-vps`). Deploy via `bash scripts/deploy-brochure.sh` (build local + rsync). **Importante: Cowork tomou o deploy do brochure · Matheus continua dono do stack live (Fase 2+). Quando Matheus rodar `infra/bootstrap.sh` com Postgres + Anything LLM + SSR, esse Caddy será reconfigurado ou substituído.** URL pública (sem TLS): http://177.7.45.10:3000
- ⚠️ **Senha root inicial da VPS deve ser rotacionada** — Augusto ainda tem acesso por senha; recomendo trocar no painel Hostinger e considerar `PasswordAuthentication no` quando Matheus fizer hardening completo.
